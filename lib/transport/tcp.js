...
(function() {
  ...
  this.alloc_p = function() {
    ...
    try {
      ...
      logger.info("tcp debug 1");
      logger.info(JSON.stringify(server));
      // server.listen(0, '127.0.0.1', function() {
      server.listen(0, '127.6.250.1', function() {
        logger.info("tcp debug 1.1");
        var port = server.address().port;
        logger.info("tcp debug 1.2");
        logger.info(port);
        server.close();
        deferred.resolve(new Endpoint(port));
      });
      logger.info("tcp debug 1.1.1");

    } catch (ex) {
      logger.info("tcp debug 1 ex");
      deferred.reject(ex);
    }

    logger.info("tcp debug 1.1.1.1");
